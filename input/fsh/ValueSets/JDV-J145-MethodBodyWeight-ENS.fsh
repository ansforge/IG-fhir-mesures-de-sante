Instance: JDV-J145-MethodBodyWeight-ENS
InstanceOf: ValueSet
Usage: #definition
* extension.url = "http://hl7.org/fhir/StructureDefinition/ressource-effectivePeriod"
* extension.valuePeriod.start = "2021-04-30T12:00:00+01:00"
* url = "https://mos.esante.gouv.fr/NOS/JDV_J145-MethodBodyWeight-ENS/FHIR/JDV-J145-MethodBodyWeight-ENS"
* identifier.value = "urn:oid:1.2.250.1.213.1.1.5.600"
* version = "20210430120000"
* name = "JDV_J145-MethodBodyWeight-ENS"
* status = #active
* date = "2021-04-30T12:00:00+01:00"
* description = "JDV pour la méthode de mesure du poids"
* compose.include.system = "https://mos.esante.gouv.fr/NOS/TRE_R306-CLADIMED/FHIR/TRE-R306-CLADIMED"
* compose.include.concept.code = #K50BI02
* compose.include.concept.display = "Balance"
* compose.include.concept.designation.use.code = #OID
* compose.include.concept.designation.value = "urn:oid:1.2.250.1.213.2.65"