# Exemple de mesure du cholestérol HDL - Guide d'implémentation FHIR - Mesures de santé v3.1.1

* [**Table of Contents**](toc.md)
* [**Artifacts Summary**](artifacts.md)
* **Exemple de mesure du cholestérol HDL**

## Example Observation: Exemple de mesure du cholestérol HDL

Profil: [Cholestérol - HDL](StructureDefinition-mesures-observation-cholesterol-hdl.md)

> **Valeur originale**
* has-been-converted: true
* original-code: Cholestérol HDL [Masse/Volume] Sérum/Plasma ; Numérique
* original-value: 1.08 g/L (Détails : code UCUMg/L = 'gram per liter')

**status**: Final

**code**: Cholestérol HDL [Masse/Volume] Sérum/Plasma ; Numérique

**subject**: [Pierre Durand](Patient-example-mes-fr-patient-001.md)

**effective**: 2024-10-15

**value**: 2.8 mmol/L (Détails : code UCUMmmol/L = 'millimole per liter')

**method**: DEG

### ReferenceRanges

| | |
| :--- | :--- |
| - | **Low** |
| * | 3.1 mmol/L (Détails : code UCUMmmol/L = 'millimole per liter') |



## Resource Content

```json
{
  "resourceType" : "Observation",
  "id" : "7b166d82-27b6-4878-9765-3fe101618edf",
  "meta" : {
    "profile" : [
      "https://interop.esante.gouv.fr/ig/fhir/mesures/StructureDefinition/mesures-observation-cholesterol-hdl"
    ]
  },
  "extension" : [
    {
      "extension" : [
        {
          "url" : "has-been-converted",
          "valueBoolean" : true
        },
        {
          "url" : "original-code",
          "valueCodeableConcept" : {
            "coding" : [
              {
                "system" : "http://loinc.org",
                "code" : "2085-9",
                "display" : "Cholestérol HDL [Masse/Volume] Sérum/Plasma ; Numérique"
              }
            ]
          }
        },
        {
          "url" : "original-value",
          "valueQuantity" : {
            "value" : 1.08,
            "unit" : "g/L",
            "system" : "http://unitsofmeasure.org",
            "code" : "g/L"
          }
        }
      ],
      "url" : "https://interop.esante.gouv.fr/ig/fhir/mesures/StructureDefinition/mesures-original-data"
    }
  ],
  "status" : "final",
  "code" : {
    "coding" : [
      {
        "system" : "http://loinc.org",
        "code" : "2085-9",
        "display" : "Cholestérol HDL [Masse/Volume] Sérum/Plasma ; Numérique"
      }
    ]
  },
  "subject" : {
    "reference" : "Patient/example-mes-fr-patient-001",
    "type" : "Patient",
    "display" : "Pierre Durand"
  },
  "effectiveDateTime" : "2024-10-15",
  "valueQuantity" : {
    "value" : 2.8,
    "unit" : "mmol/L",
    "system" : "http://unitsofmeasure.org",
    "code" : "mmol/L"
  },
  "method" : {
    "coding" : [
      {
        "system" : "https://smt.esante.gouv.fr/terminologie-tccr",
        "code" : "DEG"
      }
    ]
  },
  "referenceRange" : [
    {
      "low" : {
        "value" : 3.1,
        "unit" : "mmol/L",
        "system" : "http://unitsofmeasure.org",
        "code" : "mmol/L"
      }
    }
  ]
}

```
